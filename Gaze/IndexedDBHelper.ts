namespace Gaze {
    ...
    export class IndexedDBHelper<T> {
        ...
        public FindRowAsync(path: string, value: string | number): JQueryPromise<T> {
            let d = $.Deferred<any>();
        ...
        public FindRowsAsync(path: string, value: string | number): JQueryPromise<T[]> {
            let d = $.Deferred<any>();
            let db = this.Open();
        ...
        public UpsertAsync(entity: T | Array<T>, primaryKeyPath?: string): JQueryPromise<any> {
            let d = $.Deferred<any>();
            let db = this.Open();
